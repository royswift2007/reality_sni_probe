# REALITY SNI Probe v2.3

用于**批量评估域名是否适合作为 REALITY SNI 候选站点**的 Bash 检测脚本。

- 当前维护脚本：[`reality_sni_probe_v2.3.sh`](reality_sni_probe_v2.3.sh)

v2.3 是当前统一维护版本，集中包含 ASN 查询与分类、随机抖动、HTTP/2 探测退化修复、macOS/BSD 证书日期解析、多行 SAN 解析、导出哨兵值友好显示、OCSP 降权、证书剩余天数去评分化、数值项软 / 硬阈值、默认低并发和终端列按权重排序等能力。
...
REALITY 的工作前提是：**当外部主动探测你的节点时，服务端会向真实 SNI 目标转发握手**。探测方观察到的握手特征（延迟、证书、ALPN 等）来自 **“REALITY 服务器到真实目标”** 这条路径，而不是你本地的路径。
...
# 2. 在服务器上直接运行脚本
./reality_sni_probe_v2.3.sh -f domains.txt -o result.csv
...
- [结论判定逻辑](#结论判定逻辑)
- [评分机制与权重细节](#评分机制与权重细节)
- [ASN 类型与“大树底下好乘凉”选择原则](#asn-类型与大树底下好乘凉选择原则)
- [CDN 套用检测与权重](#cdn-套用检测与权重)
- [排序与筛选说明](#排序与筛选说明)
- [示例输出与阅读方式](#示例输出与阅读方式)
...
[`reality_sni_probe_v2.3.sh`](reality_sni_probe_v2.3.sh) 的目标，是把“一个域名能不能拿来当 REALITY 的 SNI 候选”拆成多个可量化维度，再统一汇总成结论与分数。
...
- [`probe_one()`](reality_sni_probe_v2.3.sh:1248)：单域名完整检测主流程；
- [`judge_sni()`](reality_sni_probe_v2.3.sh:922)：根据硬门槛和分支规则给出结论；
- [`calc_sni_score()`](reality_sni_probe_v2.3.sh:1010)：按当前三层权重累计分数；
- [`performance_gate_status()`](reality_sni_probe_v2.3.sh:881)：统一处理数值项软 / 硬阈值；
- [`check_h2()`](reality_sni_probe_v2.3.sh:494)：独立检测 HTTP/2；
- [`fetch_tls_bundle()`](reality_sni_probe_v2.3.sh:384)：抓取 `openssl s_client` 输出与证书 PEM；
- [`check_san_level()`](reality_sni_probe_v2.3.sh:716)：判断证书 SAN 是否精确匹配、通配匹配或不匹配；
- [`check_alpn_result_from_sclient()`](reality_sni_probe_v2.3.sh:418)：提取实际 ALPN 协商结果；
- [`check_cert_chain_status()`](reality_sni_probe_v2.3.sh:443)：判断证书链完整性；
- [`check_ocsp_stapling_from_sclient()`](reality_sni_probe_v2.3.sh:468)：判断 OCSP Stapling 状态；
- [`sample_ip_consistency()`](reality_sni_probe_v2.3.sh:632)：抽样检查多 IP 一致性；
- [`check_header_naturalness()`](reality_sni_probe_v2.3.sh:587)：评估 HTTP 响应头自然度。

---

## v2.3 更新说明

v2.3 是当前统一维护版本，核心脚本位于 [`reality_sni_probe_v2.3.sh`](reality_sni_probe_v2.3.sh)。本文不再区分未发布的中间版本，所有能力和修复都统一归入 v2.3。

### 当前 v2.3 包含的主要变化

1. **版本标识一致性**：终端标题与脚本注释统一显示 v2.3。
...
11. **TTFB 异常区间扣分加重**：`avg_ttfb > 900ms` 已经不只是“慢”，而是当前出口到目标站点的路径 / 站点自然性风险信号；v2.3 加重 `900ms+` 档位扣分，避免数百毫秒级延迟差被 CDN、多 IP 一致性这类背景加分覆盖。
...
19. **CDN 套用检测（v2.3 新增）**：通过响应头特征（`cf-ray`、`x-amz-cf-id`、`x-akamai-*`、`x-fastly-*`、`via:`、`x-served-by` 等）和 CNAME 链匹配（`*.cloudfront.net` / `*.akamai*.net` / `*.cloudflare.net` / `*.fastly.net` 等）判断目标域名是否套了第三方 CDN。归类为 `套CDN / 可能套CDN / 未套CDN / 未知`，并作为评分中**仅次于硬淘汰的大权重扣分项**：`套CDN -30`、`可能套CDN -12`。详见 [CDN 套用检测与权重](#cdn-套用检测与权重)。

这些变更不会破坏 CSV / JSONL 已有字段，只改变兼容性、退化语义、评分校准、默认并发、终端展示顺序和文档一致性。

### 为什么 v2.3 要加入 ASN 与安全优先校准

早期评分体系已经覆盖了 REALITY 最核心的几层指标（证书、SAN、TLS1.3/X25519/H2、页面形态、稳定性），但有一个明显缺口：

> **“这个域名背后的 IP 所在那条网络管道，到底是不是一棵大树？”**

这就是 ASN（Autonomous System Number，自治系统编号）要回答的问题。一个小众 ASN 上的孤立域名和一个 Cloudflare 边缘节点上的域名，哪怕 TLS 握手、证书、页面看起来都一样，被主动针对的边际代价完全不同：前者没邻居陪你挡枪，后者背后是半个互联网。

v2.3 把这层“背景噪声”量化后纳入评分，同时修掉旧版实现里的隐性问题，并加入大批量扫描时方便的辅助参数。

### 刻意没做的几件事

评估 PRD 里提到的几个方向后，以下都**确认不做**：

- **ECH（加密 ClientHello）探测**：REALITY 协议本身要求 SNI 明文，和 ECH 功能互斥。探测目标是否支持 ECH 对 REALITY 无加成，纯属噪声；
- **HTTP/3 / QUIC 真实握手**：REALITY 跑在 TCP 上，目标域名是否支持 H3 与实际效果无关。Debian 12 默认 `curl` 也不带 `--http3`；
...
例如 `tls_num = 30` 时，文档规定加 `+7`，早期实际可能累加出远高于预期的分数。v2.3 已把 `avg_tcp / avg_tls / avg_ttfb / tcp_var / tls_var / ttfb_var` 这 6 个分档块全部改写为标准 `if/elif/else` 结构。

在修复基础上，v2.3 进一步重新校准了第二层和第三层分值。核心原则：
...
- 支持直接传入多个域名；
- 支持通过文件批量读入域名；
- 自动清洗输入域名，去掉协议头、路径、端口，并转为小写，见 [`normalize_domain()`](reality_sni_probe_v2.3.sh:147)；
- 启动时会尝试选择 UTF-8 locale，并在需要时重新 `exec` 自己，减少中文表头 / 内容在不同 shell 中的乱码问题，见 [`ensure_clean_utf8_env()`](reality_sni_probe_v2.3.sh:28)；
...
- 综合形成稳定性、结论和评分；
- 支持结果表格输出，并针对中英文混排做列宽对齐修复，见 [`table_display_width()`](reality_sni_probe_v2.3.sh:1447) 与 [`table_fit_text()`](reality_sni_probe_v2.3.sh:1466)；
- 支持 CSV / JSONL 导出；
- 支持仅保留“推荐 / 可用”，以及按最小分数过滤；
- 会对输入域名去重，避免重复检测，见 [`dedup_domains()`](reality_sni_probe_v2.3.sh:1629)。
...
用于 HTTP/HTTPS 采样，见 [`check_curl_once()`](reality_sni_probe_v2.3.sh:319) 与 [`check_h2()`](reality_sni_probe_v2.3.sh:494)。
...
用于 TLS 握手信息与证书解析，见 [`fetch_tls_bundle()`](reality_sni_probe_v2.3.sh:384)、[`cert_text_from_pem()`](reality_sni_probe_v2.3.sh:408)、[`days_to_expiry_from_pem()`](reality_sni_probe_v2.3.sh:696)。

缺失时：

- 证书状态、SAN、到期时间、Issuer、TLS1.3、X25519、ALPN、证书链、OCSP 都会退化；
- [`judge_sni()`](reality_sni_probe_v2.3.sh:922) 会把 `cert_ok != 正常` 直接降为“不建议”。

### 3. 可选依赖

#### `timeout` / `gtimeout`

脚本通过 [`safe_timeout()`](reality_sni_probe_v2.3.sh:107) 包装 `openssl s_client` 和 whois 调用。若系统存在 `timeout` 或 `gtimeout`，相关阶段会被超时保护；若不存在，就直接执行命令。

#### `date -d` / `date -j` / `python3`

到期剩余天数计算由 [`days_to_expiry_from_pem()`](reality_sni_probe_v2.3.sh:696) 完成。v2.3 会优先尝试 GNU `date -d`，再尝试 BSD/macOS `date -j -f`，最后在存在 `python3` 时用 Python 兜底解析。
...
脚本需要从当前服务器直接访问目标域名的 443 端口。若当前服务器有透明中间件、企业 MITM、网络策略干预、DNS 污染或线路绕路，结果会反映这些真实路径问题。
...
将 [`reality_sni_probe_v2.3.sh`](reality_sni_probe_v2.3.sh) 放到服务器上，然后：

```bash
chmod +x reality_sni_probe_v2.3.sh
```
...
```bash
./reality_sni_probe_v2.3.sh www.microsoft.com www.cloudflare.com www.apple.com
```

### 2. 从文件批量检测

```bash
./reality_sni_probe_v2.3.sh -f domains.txt
```
...
这些输入会被 [`normalize_domain()`](reality_sni_probe_v2.3.sh:147) 统一规整为纯域名。
...
```bash
./reality_sni_probe_v2.3.sh -f domains.txt -j 2 --timeout 12 --samples 5
```

### 4. 仅保留较好结果

```bash
./reality_sni_probe_v2.3.sh -f domains.txt --only-good
```

`--only-good` 对应 [`filter_results()`](reality_sni_probe_v2.3.sh:1614)，只保留“推荐”和“可用”。

### 5. 按最小分数过滤

```bash
./reality_sni_probe_v2.3.sh -f domains.txt --min-score 90
```

### 6. 导出 CSV / JSONL

```bash
./reality_sni_probe_v2.3.sh -f domains.txt -o result.csv --json result.jsonl
```
...
./reality_sni_probe_v2.3.sh -f domains.txt -j 2 --jitter 300

# 没装 whois 或不想查 ASN 时跳过
./reality_sni_probe_v2.3.sh -f domains.txt --no-asn
```
...
脚本参数定义见 [`usage()`](reality_sni_probe_v2.3.sh:124) 与 [`main()`](reality_sni_probe_v2.3.sh:1633)。

| 参数 | 含义 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `-f FILE` | 域名文件 | 无 | 从文件逐行读取域名，忽略空行与 `#` 注释行 |
| `-o FILE` | 导出 CSV | 空 | 调用 [`export_csv()`](reality_sni_probe_v2.3.sh:1578) |
| `--json FILE` | 导出 JSONL | 空 | 调用 [`export_jsonl()`](reality_sni_probe_v2.3.sh:1597) |
| `-j NUM` | 并发数 | `1` | 写入 `JOBS`，通过 [`run_with_limit()`](reality_sni_probe_v2.3.sh:1417) 控制后台任务数；默认低并发优先保证检测质量 |
...
脚本会在 [`main()`](reality_sni_probe_v2.3.sh:1633) 中做基础校验：
...
脚本在正式执行前，会先调用 [`ensure_clean_utf8_env()`](reality_sni_probe_v2.3.sh:28)：

1. 通过 [`select_utf8_locale()`](reality_sni_probe_v2.3.sh:4) 优先从当前 `LC_CTYPE`、`LANG` 中挑选 UTF-8 locale；
...
单域名主流程由 [`probe_one()`](reality_sni_probe_v2.3.sh:1248) 驱动，可概括为：

1. 重复调用 [`check_curl_once()`](reality_sni_probe_v2.3.sh:319) 进行 `SAMPLES` 次 HTTPS 请求；
2. 提取 TCP 建连、TLS 握手、TTFB、HTTP 版本、状态码、内容类型、响应体大小、最终 URL、页面标题、响应头摘要、远端 IP；
3. 计算平均值与三项抖动；
4. 选择首个有效 HTTP 样本作为页面 / WAF / 跳转 / 头部分析基准；
5. 通过 [`fetch_tls_bundle()`](reality_sni_probe_v2.3.sh:384) 运行带 `-status` 与 `-alpn 'h2,http/1.1'` 的 `openssl s_client`；
6. 解析证书、TLS1.3、X25519、ALPN、证书链、SAN、OCSP、证书指纹、到期时间、颁发者；
7. 判断 HTTP/2 支持情况；
8. 进行页面自然度、响应头自然度、WAF、跳转自然度判断；
9. 调用 [`sample_ip_consistency()`](reality_sni_probe_v2.3.sh:632) 对最多 3 个 A 记录做一致性复核；
10. 查询 ASN 并分类；
11. 调用 [`judge_sni()`](reality_sni_probe_v2.3.sh:922) 产生“推荐 / 可用 / 勉强 / 不建议”；
12. 调用 [`calc_sni_score()`](reality_sni_probe_v2.3.sh:1010) 叠加权重分；
13. 过滤、排序、展示 / 导出。
...
见 [`check_curl_once()`](reality_sni_probe_v2.3.sh:319)。脚本读取：
...
见 [`check_h2()`](reality_sni_probe_v2.3.sh:494)。如果 `curl` 不支持 HTTP/2，则返回“未知”；主流程里若 OpenSSL ALPN 已确认 `h2`，会用 ALPN 兜底记为“支持”。

### 3. `openssl s_client` TLS 抓取

见 [`fetch_tls_bundle()`](reality_sni_probe_v2.3.sh:384)。命令形态为：
...
- [`cert_text_from_pem()`](reality_sni_probe_v2.3.sh:408)：解析完整证书文本；
- [`check_cert_ok()`](reality_sni_probe_v2.3.sh:413)：判断是否拿到 PEM；
- [`get_expiry_from_pem()`](reality_sni_probe_v2.3.sh:689)：读取到期时间；
- [`days_to_expiry_from_pem()`](reality_sni_probe_v2.3.sh:696)：计算剩余天数；
- [`get_issuer_short_from_pem()`](reality_sni_probe_v2.3.sh:709)：读取颁发者；
- [`check_san_level()`](reality_sni_probe_v2.3.sh:716)：判断 SAN 覆盖级别。

### 5. 页面、WAF、跳转、头部、多 IP

- [`page_naturalness()`](reality_sni_probe_v2.3.sh:817)：页面形态；
- [`detect_waf_challenge()`](reality_sni_probe_v2.3.sh:799)：疑似挑战 / 拦截；
- [`redirect_naturalness()`](reality_sni_probe_v2.3.sh:771)：跳转自然度；
- [`check_header_naturalness()`](reality_sni_probe_v2.3.sh:587)：响应头自然度；
- [`sample_ip_consistency()`](reality_sni_probe_v2.3.sh:632)：多 IP 一致性。
...
终端表格列定义见 [`TABLE_HEADERS`](reality_sni_probe_v2.3.sh:1438)。当前 v2.3 默认显示 25 列：
...
其中 `码 / TCP建连 / TLS握手 / TTFB / 平均延迟 / 抖动T/TLS/F / TLS13 / X25519 / H2 / ALPN` 的顺序保持原样；后面的站点判断列按当前权重与 REALITY 安全优先级从高到低排列。`CDN` 列位于 `OCSP` 之后、`评分` 之前，便于把"是否套 CDN"作为评分前的最后一道人工核查。
...
| `OCSP` | 最低 | v2.3 已降为轻量信号；评分中 `支持 +2`、`未提供 0`、`异常 -3`、`未知 0`。 |

实用理解：

- 分数接近时，优先选择左侧关键列更好的候选，而不是只看 `OCSP` 这种轻量信号。
- `ASN类型=CDN`、`多IP=一致` 这类背景项通常比 `OCSP=支持` 更重要，因为 REALITY 更看重“大树底下好乘凉”的背景环境。
...
在 [`probe_one()`](reality_sni_probe_v2.3.sh:1248) 输出的完整 TSV 中，脚本实际还包含：
...
在当前实现中，`平均延迟` 实际上直接等于 `avg_ttfb`，见 [`probe_one()`](reality_sni_probe_v2.3.sh:1314)。
...
| 字段 | 含义 |
| --- | --- |
| `domain` | 输入域名，经 [`normalize_domain()`](reality_sni_probe_v2.3.sh:147) 规整后的最终检测目标。 |
...
| `tls13` | 由 [`check_tls13_from_sclient()`](reality_sni_probe_v2.3.sh:484) 判断。 |
| `x25519` | 由 [`check_x25519_from_sclient()`](reality_sni_probe_v2.3.sh:489) 判断。 |
| `h2` | HTTP/2 支持状态，可能来自 `curl` 样本、OpenSSL ALPN 兜底或 [`check_h2()`](reality_sni_probe_v2.3.sh:494)。 |
| `alpn_result` | 实际 ALPN 协商结果，来自 [`check_alpn_result_from_sclient()`](reality_sni_probe_v2.3.sh:418)。 |
| `cert_ok` | 是否成功获取证书 PEM，来自 [`check_cert_ok()`](reality_sni_probe_v2.3.sh:413)。 |
| `cert_chain_status` | 证书链状态，来自 [`check_cert_chain_status()`](reality_sni_probe_v2.3.sh:443)。 |
| `san_level` | SAN 覆盖级别，来自 [`check_san_level()`](reality_sni_probe_v2.3.sh:716)。 |
| `ocsp_stapling` | OCSP Stapling 状态，来自 [`check_ocsp_stapling_from_sclient()`](reality_sni_probe_v2.3.sh:468)。 |
| `page` | 页面自然度，来自 [`page_naturalness()`](reality_sni_probe_v2.3.sh:817)。 |
| `header_naturalness` | HTTP 响应头自然度，来自 [`check_header_naturalness()`](reality_sni_probe_v2.3.sh:587)。 |
| `waf` | WAF / 挑战页判断，来自 [`detect_waf_challenge()`](reality_sni_probe_v2.3.sh:799)。 |
| `redirect` | 跳转自然度，来自 [`redirect_naturalness()`](reality_sni_probe_v2.3.sh:771)。 |
| `ip_consistency` | 多 IP 一致性状态，来自 [`sample_ip_consistency()`](reality_sni_probe_v2.3.sh:632)。 |
| `stability` | 稳定性等级，来自 [`stability_level()`](reality_sni_probe_v2.3.sh:846)。 |
| `score` | 综合评分，来自 [`calc_sni_score()`](reality_sni_probe_v2.3.sh:1010)。 |
| `result` | 最终结论，来自 [`judge_sni()`](reality_sni_probe_v2.3.sh:922)。 |
| `issuer` | 证书颁发者摘要。 |
...
最终结论由 [`judge_sni()`](reality_sni_probe_v2.3.sh:922) 给出。
...
其中 `redirect / TLS13 / X25519 / H2 / SAN` 这组 Reality 核心硬条件由 [`is_reality_hard_fail()`](reality_sni_probe_v2.3.sh:861) 统一判断。

### 2. 数值项硬阈值：直接“不建议 / -9999”

由 [`performance_gate_status()`](reality_sni_probe_v2.3.sh:881) 统一判断：
...
命中数值硬阈值时，[`calc_sni_score()`](reality_sni_probe_v2.3.sh:1010) 输出 `-9999`。
...
当前评分由 [`calc_sni_score()`](reality_sni_probe_v2.3.sh:1010) 计算，整体分为：
...
- 某些很差的候选可以出现负分；
- 命中 [`is_reality_hard_fail()`](reality_sni_probe_v2.3.sh:861) 或 [`performance_gate_status()`](reality_sni_probe_v2.3.sh:881) 硬阈值的记录，评分会直接输出 `-9999`；
- 某些优质候选可以超过 `100`；
- 它更适合作为**同批候选之间的排序依据**。

---

## ASN 类型与“大树底下好乘凉”选择原则

ASN（Autonomous System Number）可以粗略理解为：互联网上“谁家管哪一片 IP 地址”。一个域名最终落在哪个 ASN 上，会影响它是不是像一个“大树底下”的正常大流量站点。

### 为什么 REALITY 要关心 ASN

REALITY 的安全目标不是“找一个能访问的域名”，而是“找一个被主动探测时看起来足够自然、针对成本足够高的目标”。
...
v2.3 新增了"目标域名是否套了第三方 CDN"的探测。这一节解释为什么需要这个检测、检测怎么做的、以及为什么权重设置成大扣分项。

### 为什么 SNI 选择需要关心是否套 CDN

REALITY 的工作前提是：**外部主动探测时，你的服务端会向真实 SNI 目标 dial 一次 TLS 握手，把真品证书返回给探测方**。

这个流程里有两条关键假设：

1. **你的服务器 dial 到的目标行为**和**探测方 dial 到的目标行为**应当尽量一致；
2. **同一个 SNI 目标在不同时间、不同地区都返回稳定一致的证书 / TLS 行为**。

套了 CDN 的域名会在这两条假设上引入风险：

- **Anycast 路由不一致**：CDN 边缘节点全球分布，你的服务器命中的 PoP 和探测方命中的 PoP 可能不同；
- **TLS 行为可能有细微差异**：不同 PoP 跑的 TLS 栈版本、cipher 偏好、session ticket 行为可能不一致；
- **CDN 的 rate limit / 异常处理**：CDN 检测到"同一 IP 频繁 TLS 握手但不发 HTTP 请求"时可能触发降级行为；
- **回落路径复杂**：套 CDN 的域名在握手细节和源站直连不完全一致，影响 REALITY 协议身份的真实性。

这是 REALITY 社区里**有争议、无权威定论**的问题。部分观点认为 CDN 域名背景流量大、针对成本高所以适合做 SNI；另一部分观点认为套 CDN 的域名在握手一致性上风险大，应避开。

**v2.3 选择保守路线**：默认把"套 CDN"视为风险信号，给予明显扣分。如果你有自己的判断，可以从 CSV / JSONL 的 `cdn_status` 字段拿到原始数据自行决策。

### 检测怎么做的

`detect_cdn()` 函数（见 [`reality_sni_probe_v2.3.sh`](reality_sni_probe_v2.3.sh)）综合三层信号：

#### 第一层：HTTP 响应头精确匹配（最可靠）

| CDN 厂商 | 关键响应头 |
| --- | --- |
| Cloudflare | `cf-ray:`、`cf-cache-status:`、`server: cloudflare` |
| AWS CloudFront | `x-amz-cf-id:`、`x-amz-cf-pop:`、`via:` 含 `cloudfront.net` |
| Akamai | `x-akamai-*`、`server: AkamaiGHost`、`akamai-grn:` |
| Fastly | `x-fastly-*`、`x-served-by: cache-*`、`via:` 含 `varnish` |
| 通用 CDN | `x-cdn:`、`x-edge-*`、`x-tencent-*`、`x-cos-*` |

每命中一组特征 +2 分（精确匹配）或 +1 分（通用）。

#### 第二层：CNAME 链匹配（DNS 层）

通过 `dig` 解析域名的 CNAME 链，判断是否指向已知 CDN 服务商：

- `*.cloudfront.net`、`*.cloudflare.net`、`*.cdn.cloudflare`
- `*.akamai.net`、`*.akamaiedge.net`、`*.edgekey.net`、`*.edgesuite.net`
- `*.fastly.net`、`*.fastlylb.net`
- `cdn77.*`、`stackpathdns.*`、`impervadns.*`、`incapdns.*`

CNAME 命中 +2 分。

#### 第三层：通用缓存头（弱信号）

`age:`、`x-cache:`、`x-served-by:`、`x-cache-hits:` 等通用缓存头命中视为弱信号，只在前两层都没命中时影响判定。
...
- 硬淘汰：直接 `-9999`（`SAN=不匹配 / 无SAN`、`TLS13=不支持`、`X25519=不支持`、`H2=不支持`、`跨站跳转`、证书到期 < 14 天）
- `ttfb_var > 650ms`：−45
- `san_level=不匹配`（已硬淘汰）：−42
- **`cdn_status=套CDN`：−30**
- `waf=疑似挑战`：−28
- `avg_ttfb > 1600ms`：−35

`套CDN` 的扣分量级仅次于硬淘汰条件，足以把"套了 CDN 的高分候选"明显压低，但不会把它直接淘汰——因为这不是 REALITY 的硬性禁忌，只是有争议的风险点。

### 看到 `套CDN` 该怎么处理

实战建议：

1. **优先选 `未套CDN` 的高分候选**作为 SNI；
2. **`可能套CDN` 可作次选**，特别是分数接近时；
3. **`套CDN` 候选谨慎使用**——如果其他指标极优（推荐档、精确匹配、ASN 大、稳定性好），可以接受；如果有更好的非 CDN 替代，优先换；
4. **验证你的 REALITY 服务器是否和 CDN 同 ASN**——如果是，反而是合理画像（比如 AWS 服务器访问 CloudFront）；
5. **混合 SNI 池**——不要全部依赖 CDN 域名或全部依赖非 CDN 域名，多样化更稳。

### 局限性

- **CDN 厂商总在变化**：脚本只识别主流 CDN（Cloudflare / CloudFront / Akamai / Fastly / 部分国内厂商）。新兴 CDN 或私有 CDN 可能被漏判为 `未套CDN` 或 `未知`；
- **CNAME 解析依赖 `dig`**：缺 `dig` 时退化为只看响应头；
- **响应头可能被改写**：少数站点会移除 `via:` / `server:` 等头，导致漏判；
- **大厂自营 CDN 难以区分**：`m.media-amazon.com` 是 Amazon 自营 CloudFront、`www.apple.com` 可能挂 Apple 自营 Akamai——这些"自家挂自家 CDN"在脚本里仍会被判 `套CDN`。这是技术上无法区分的。如果你认为大厂自营 CDN 应该宽容处理，可以用 CSV 字段自行筛选。
...
脚本会先调用 [`filter_results()`](reality_sni_probe_v2.3.sh:1614)：
...
过滤完成后，脚本在 [`main()`](reality_sni_probe_v2.3.sh:1633) 中按以下键排序：

1. **REALITY 核心硬条件**是否满足（非跨站跳转 + TLS13/X25519/H2 支持 + SAN 精确或通配匹配）；
2. **结论档位**（推荐 > 可用 > 勉强 > 不建议）；
3. **SAN 等级**（精确匹配 > 通配匹配 > 其他）；
4. **`score` 数值**；
5. TLS 抖动；
6. TTFB 抖动。
...
- **协议硬条件**最优先——不满足的直接沉底；
- **同硬条件内，按结论档位分组**——把"可用"和"勉强"明确分开，避免高分"勉强"挤掉低分"可用"；
- **同档位内，精确匹配优于通配匹配**——身份匹配越精确，REALITY 风险越低；
- **同 SAN 等级内，按分数排**——最直观的细分维度；
- **最后用抖动做 tie-break**——分数相同就看更稳定的。

举例（来自实测数据）：

- `勉强档 + 精确匹配 + 分数 26`（cdn-dynmedia）会排在 `勉强档 + 通配匹配 + 分数 89`（hkex）之前——因为档位相同、SAN 等级（精确）的安全权重高于分数差；
- `可用档 + 精确匹配 + 分数 108`（speedtest）会排在 `勉强档 + 精确匹配 + 分数 136`（chuo-u）之前——因为档位差异（可用 > 勉强）优先于分数差。
...
当前导出字段由 [`export_csv()`](reality_sni_probe_v2.3.sh:1578) 与 [`export_jsonl()`](reality_sni_probe_v2.3.sh:1597) 决定，顺序如下：
...
33. `asn`
34. `asn_type`
35. `cdn_status`（v2.3 新增）：`套CDN / 可能套CDN / 未套CDN / 未知`
36. `cdn_name`（v2.3 新增）：识别到的 CDN 厂商名（如 `Cloudflare` / `CloudFront` / `Akamai` / `Fastly`），未识别为 `-`
...
- 若底层抖动值为 `999999`，v2.3 导出会写成 `-`，表示“该项抖动不可得”；
...
1. [`check_cert_ok()`](reality_sni_probe_v2.3.sh:413) 只判断“有没有拿到 PEM”，不等于完整证书链校验通过；
2. [`check_cert_chain_status()`](reality_sni_probe_v2.3.sh:443) 与 [`check_ocsp_stapling_from_sclient()`](reality_sni_probe_v2.3.sh:468) 都基于 `openssl s_client` 文本输出，不是结构化 PKI / OCSP 验证器；
3. [`check_tls13_from_sclient()`](reality_sni_probe_v2.3.sh:484) 和 [`check_x25519_from_sclient()`](reality_sni_probe_v2.3.sh:489) 是基于文本匹配，不是结构化 TLS 指纹分析；
...
因为当前 [`calc_sni_score()`](reality_sni_probe_v2.3.sh:1010) 的主权重已经放在证书、SAN、TLS1.3、X25519、ALPN、证书链这些安全与身份特征上。速度快只能在第三层补充分中加分，无法弥补根本性的证书或协议短板。OCSP 仍会参与评分，但已下调为轻量信号；`expiry_days` 只保留 `< 14` 天临期硬保护，不再参与评分排序。

### 2. 为什么 `通配匹配` 不再直接判死？

因为当前 [`judge_sni()`](reality_sni_probe_v2.3.sh:922) 已把 `通配匹配` 调整为“次优可用”。它不能拿到和 `精确匹配` 一样高的权重，但如果其他条件好，仍可得到“可用”；如果页面或稳定性较弱，则多半回落为“勉强”。

### 3. 为什么 `403` 有时还是“可用”或“勉强”？

因为当前 [`judge_sni()`](reality_sni_probe_v2.3.sh:922) 对 `403` 的处理是：只要没有提前命中证书 / SAN / 到期 / Reality 核心硬性淘汰条件，`403` 仍可能落入“可用”或“勉强”。这反映的是“可作为候选参考”，不等于“用户访问体验优秀”。

### 4. 为什么默认终端里能看到 `ALPN`、`OCSP`、`链`、`头部`、`多IP`，而且后半段列顺序变了？

因为当前 [`print_table()`](reality_sni_probe_v2.3.sh:1539) 的表头已经扩展，新增这些判定列，同时通过 [`table_display_width()`](reality_sni_probe_v2.3.sh:1447) 与 [`table_fit_text()`](reality_sni_probe_v2.3.sh:1466) 修复了中英文混排时的表格对齐问题。

v2.3 后半段站点判断列现在按阅读权重排列为：`SAN → 证书 → 跳转 → WAF → 页面 → 稳定性 → ASN类型 → 多IP → 链 → 头部 → OCSP`。这样分数接近时，不熟悉各项权重的用户也可以优先看左侧更重要的项，再看右侧轻量辅助项。

### 5. 为什么“平均延迟”和 `TTFB` 一样？

因为当前实现中 [`probe_one()`](reality_sni_probe_v2.3.sh:1248) 直接把 `avg_ttfb` 复用为 `avg_latency` 输出，它目前就是展示别名，不是新的独立指标。
...
### 7. v2.3 为什么仍没有做 ECH / HTTP/3 / PQC 检测？

因为对 REALITY 的实际帮助极小，代价却很大：

- **ECH**：REALITY 要求 SNI 明文，和 ECH 的设计目标互斥；
- **HTTP/3 / QUIC**：REALITY 跑在 TCP 上，目标是否支持 H3 和实际效果无关；
- **PQC**：这是 Xray / utls 客户端层要解决的问题，不是 SNI 探测职责。

### 8. 我该用旧版还是 v2.3？

默认推荐 **v2.3**。它包含 ASN 查询与分类、HTTP/2 退化修复、SAN 多行解析、macOS/BSD 日期解析、导出哨兵值友好显示、默认低并发、数值阈值和安全优先评分权重。

继续用旧版的唯一合理理由是：下游消费脚本严格依赖旧版某个具体的绝对分数阈值，且短期内不方便重新校准。

### 9. v2.3 跑同一个域名，为什么在不同服务器上分数不一样？
...
最可靠的验证环境是 Debian 12 / Ubuntu 22.04+ 上 stock `curl + openssl`。如果打算长期部署，在你实际跑 REALITY 的那台服务器上跑 v2.3 得到的结果才算数。
